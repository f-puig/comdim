SelectFeaturesRB <- function(RB = RB, results = results, ndim = NULL, blocks = NULL, threshold_cor = 1, threshold_cov = 1, mean.RB = T, plots = "NO") {

#' SelectFeaturesRB
#'
#' Finds the important variables presenting a coordinated response in all the replicate-blocks
#' @param RB The object used as input in ComDim_PCA().
...
#' @param mean.RB Logical value to indicate whether the RB data must be mean-centered (TRUE) or not (FALSE).
...
#'  features <- SelectFeaturesRB(RB = rb, results = results, ndim = 1, blocks = c(2,3,4))
#' @export
  ...
  ntable = length(RB) # number of blocks

  # Checking that RB is a suitable multi-block structure
  ...
  if (length(RB) < max(blocks)){
    stop("The list in 'RB' is incorrect.")
  }

  for (i in 1:ntable){

    sample_number[i] <- length(RB[[1]]$Samples)
    variable_number[i] <- length(RB[[i]]$Variables)
    if(sum(properties %in% names(RB[[i]])) != 3){ # Checking if a field is missing
      print(sprintf('Block %s has missing fields.', i))
      give_error <- 1
    }

    if (nrow(RB[[i]]$Data) != length(RB[[i]]$Samples)) {
      print(sprintf('Block %s has incorrect number of samples.', i))
      give_error <- 1
    }

    if (ncol(RB[[i]]$Data) != length(RB[[i]]$Variables)) {
  ...
  if(!("Q" %in% names(results))){
    ...
    if(nrow(results$Q$Data) != unique(sample_number)){
      stop("The size of the local scores do not match the size of 'RB'.")
    }
  }
  ...
  if (!("P_Loc" %in% names(results))){
    ...
    if (length(results$P_Loc$Data) < max(blocks)){
      ...
      for (i in 1:length(blocks)){
        ...
        if(nrow(results$P_Loc$Data[[blocks[i]]]) != ncol(RB[[blocks[i]]]$Data)){
          stop("The size of the local loadings do not match the size of 'RB'.")
        }
      }
    }
  }

  if (!(is.logical(mean.RB))){
    stop("The parameter 'mean.RB' must be logical.")
  }
  ...
  common_RB_pos <- list(NULL)
  common_RB_neg <- list(NULL)
  for(i in 1:length(blocks)){
    if (i == 1){
      common_RB_pos <- imp_pos[[i]]
      common_RB_neg <- imp_neg[[i]]
    } else {
      common_RB_pos <- intersect(common_RB_pos, imp_pos[[i]])
      common_RB_neg <- intersect(common_RB_neg, imp_neg[[i]])
    }
  }

  names(common_RB_pos) <- RB[[blocks[1]]]$Variables[common_RB_pos]
  names(common_RB_neg) <- RB[[blocks[1]]]$Variables[common_RB_neg]
  ivs <- list (positive = common_RB_pos, negative = common_RB_neg)
  ...
  if (grepl('NO', plots, ignore.case = TRUE)) {
    ...
    for(i in 1:length(blocks)){
      ...
      labels[c(imp_pos[[i]],imp_neg[[i]])] <- RB[[blocks[i]]]$Variables[c(imp_pos[[i]],imp_neg[[i]])]
    ...
    for(i in 1:length(blocks)){
      ...
      labelsi[[i]][c(imp_pos[[i]],imp_neg[[i]])] <- RB[[blocks[i]]]$Variables[c(imp_pos[[i]],imp_neg[[i]])]
